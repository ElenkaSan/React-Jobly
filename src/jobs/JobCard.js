...
function JobCard({ id, title, salary, equity, companyName, remove }) {
    ...
    <section>
      <Card className="JobCard card"> {applied}
        <CardBody>
          ...
          <CardSubtitle>
            <NavLink className="text-info" to={`/companies/${companyName}`}> 
              Company: {companyName}
            </NavLink>
          </CardSubtitle>
          <br></br>
        ...
        <CardBody className="job-card-body">
          <button
            ...
            {applied ? "Applied" 
            && ( <button className='btn text-info btn-outline-warning btn-sm' style={{ }} onClick={() =>remove({ id: hasAppliedToJob.id })}> Remove from Applied </button> )
            : "Apply"}
          </button>
        </CardBody>
      </Card>
